# -*- coding: utf-8 -*-
#import csv as csv
from models import Zone,ServerInfo,Service,Location,Firewall,Switch,Upload
from xlrd import *
...
def SaveFirewall(lst):
	try:
		logok=''
		logero=''
		first_sheet = lst.sheet_by_index(0)
		for r in range(1, first_sheet.nrows):
			m 	= first_sheet.cell(r,3).value
			m1 	= first_sheet.cell(r,4).value
			m2 	= first_sheet.cell(r,2).value
			m3 	= ServerInfo.objects.get(IP=first_sheet.cell(r,6).value).id
			chk 	= Firewall.objects.filter(DestinationIP=m1,Port=m2,Server_id=m3,SourceIP=m).exists()	
			if chk == False:
				server = ServerInfo.objects.get(IP=first_sheet.cell(r,6).value)
				sv=Firewall(
					Allow 			= first_sheet.cell(r,0).value,
					Way 			= first_sheet.cell(r,1).value,
					Port 			= first_sheet.cell(r,2).value,
					SourceIP 		= m,
					DestinationIP 	= first_sheet.cell(r,4).value,
					Description 	= first_sheet.cell(r,5).value,
					Server_id 		= server.id,
					)
				sv.save()
				logok += m.encode("utf-8")+'/'
